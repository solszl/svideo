...
export default class VideoDemuxer extends AbstractDemuxer {
  ...
  _parseAVCPacket(frameType) {
    ...
    case 0:
    {
      ...
      DataStore.OBJ.metaEndPosition = position + Buffer.readAsInt(tagSize) + 4; // 缓存scriptTag结束的位置，replay使用
  ...
  _parseAVCDecoderConfigurationRecord() {
    ...
    if (meta) {
      ...
      DataStore.OBJ.videoMetaData = {};
      meta = {};
      meta.type = 'video';
      meta.id = track.id;
      meta.timeScale = timeScale;
      meta.duration = duration;
      mi.timescale = timeScale;
    }
    ...
    DataStore.OBJ.naluLengthSize = dv.getUint(2, this.readOffset, false) + 1;
    const naluLengthSize = DataStore.OBJ.naluLengthSize;
  ...
  _parseAVCVideoData(frameType, cpsTime) {
    ...
    let dataLen = 0;
    const {
      naluLengthSize: naluLenSize,
      timeStampBase
    } = DataStore.OBJ;
    let ts = timeStampBase + this.currentTag.getTime();
    // console.log(ts, this._store.state.timeStampBase, this.currentTag.getTime());
