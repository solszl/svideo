...
class IOController {
  ...
  _onContentLengthKnown(contentLength) {
    if (contentLength && this._fullRequestFlag) {
      ...
      store.setKV(KV.FileSize, contentLength)
    }
  }
  ...
  _onLoaderChunkArrival(chunk, byteStart, receivedLength) {
    ...
    // 统计下载量
    const { store } = this._config
    if (!store.getKV(KV.DownloadSize)) {
      store.setKV(KV.DownloadSize, 0)
    }
    let size = store.getKV(KV.DownloadSize)
    size += chunk.byteLength
    store.setKV(KV.DownloadSize, size)
