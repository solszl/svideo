  ...
  appendInitSegment(segment, deferred) {
    ...
    if (mimeType !== this._mimeType[seg.type]) {
      if (!this._mimeTypes[seg.type]) {
        ...
        this.info('info', `Notice: ${seg.type} mimeType changed, origin: ${this._mimeTypes[seg.type]}, target: ${mimeType}`);
      }
    ...
    if (Browser.isSafari && seg.container === 'audio/mpeg' && seg.mediaDuration > 0) {
      // 'audio/mpeg' 轨道在safari下 可能 导致时长为 NaN
    }
  }
